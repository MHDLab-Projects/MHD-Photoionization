...
\begin{figure}[h]
    ...
    \caption{Experimental setup of the Photoionization experiment. D) Time-resolved photolumniscence images from the ICCD camera. TODO: the images are currently averaged over the entire run (streaks are different positions) }
...
For the results presented in this report the total mass flow rate is held constant at 13 g/s. In this report, we focus on three sequences where the potassium mass fraction, the laser power, and the position of the laser downstream from the exit of the free jet are varied. (TODO: incorporate phi variation). Notably, when not being varied, the position of the AES/MWS diagnostics is held fixed downstream of the torch at a 'nominal position' of 178 mm downstream of the exit of the HVOF torch (TODO: improve). The motivation for this position is discussed in the results section.  
