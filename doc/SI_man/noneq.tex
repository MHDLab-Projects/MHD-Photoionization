...
\begin{equation}
  R = - \sum_{i}^{}k_{r, b, i}n_{e}(t)n_{i}(t)
\end{equation}
...
\begin{equation}
  ...
  R = - \sum_{i}^{}[k_{r, b, i}n_{e,0}n_{i,0}  + k_{r, b, i}n_{i,0}\Delta n_{e}(t) + k_{r, b, i}n_{e,0}\Delta n_{i}(t) + k_{r, b, i}\Delta n_{e}(t)\Delta n_{i}(t)]
\end{equation}


If we consider Equation \ref{eq:start_deqn} in the equilibrium case where $\frac{dn_{e}}{dt} = 0$ and $G_{NE} = 0$, we can see that the first time independent term in \ref{eq:recomb_expand} is equal to $G_{th}$. Therefore this time independent term will cancel out in Equation \ref{eq:start_deqn} and we can write the full differential equation as

\begin{equation}
  \label{eq:full_deqn}
\frac{d\Delta n_{e}(t)}{dt} = G_{NE}  -  \sum_{i}^{}[k_{r, b, i}n_{i,0}\Delta n_{e}(t) + k_{r, b, i}n_{e,0}\Delta n_{i}(t) + k_{r, b, i}\Delta n_{e}(t)\Delta n_{i}(t)]
\end{equation}

We are now going to make a series of simplifying assumptions to this equation. These assumptions are geared towards the situation of a small perturbation of the electron density from equilibrium, where $\Delta n_{e}(t) << n_{e,0}$. Experimentally, we observe that the MWS signal eventually enters an exponentially decaying regime, which we interpret as the electron density entering this small-perturbation regime with $G_{NE} =0$ (the laser pulse is over). In this case, we know that the system enters a state where the differential equation becomes of an exponential form:

\begin{equation}
  \label{eq:small_pert_diffeq}
\frac{d\Delta n_{e}(t)}{dt} = k_{r,m,eff} \Delta n_{e}(t)
\end{equation}

Where $k_{r,m,eff}$ is the effective monomolecular recombination rate constant. This observation implies that $\Delta n_i(t)$ is proportional to $\Delta n_{e}(t)$, and we can write

\begin{equation}
  \label{eq:phi_def}
\Delta n_i(t) = \phi_i \Delta n_{e}(t)
\end{equation}

Where $\phi_i$ is a proportionality constant and $\sum_{i}^{}\phi_i = 1$ from charge neutrality. We can then substitute this into Equation \ref{eq:full_deqn} to get

\begin{equation}
  \label{eq:full_deqn_phi}
\frac{d\Delta n_{e}(t)}{dt} = G_{NE} - k_{r, m, eff}\Delta n_{e}(t) - k_{r, b, eff} \Delta n_{e}(t)^2
\end{equation}
...
With the earlier stated assumption that $\Delta n_{e}(t) << n_{e,0}$, we can see that the third (bimolecular) term in Equation \ref{eq:full_deqn_phi} indeed becomes Equation \ref{eq:small_pert_diffeq} in the case of $G_{NE} = 0$.
...
The $\phi_i$ are not known \emph{a priori} and are not easily measured. We are going to make the simplifying assumption that  $\phi_K+ = 1$ and $\phi_i = 0$ for all other species. This assumption allows us to write.   
