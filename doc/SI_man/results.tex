...
\begin{equation}
    \label{eq:krbeff}
    k_{r,b,eff} = \frac{1}{\tau_{experiment} * n_{all} * n_{O2}}
\end{equation}

where $\tau_{experiment}$ is the measured decay time, $n_{all}$ is the total number density, and $n_{O2}$ is the oxygen number density predicted from CFD. Figure \ref{fig:SI_krb_eff_53x} shows the calculation of the effective recombination coefficient for 53x.
