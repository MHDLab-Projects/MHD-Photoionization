...
For the exponential fit the data is down selected to a window of 5 to 15 $\mu s$ after the laser pulse and fit with an exponential fit model $y = A \exp(-t/\tau) + C$.

For the solution to the differential equation we assume a $k_{r,b,K+} = 1 \times 10^{-8} cm^3/s$ (TODO improve accuracy and source). The differential equation is solved with the scipy solve\_ivp function. The solution and the $\Delta AS$ curve are both normalized and normalized solution is fit to the normalized $\Delta AS$ curve with the lmfit package.
...
Before each experiment we performed measurements with a silicon sample in the location of the torch to ensure the system was functioning properly. The silicon measurements are shown in \ref{fig:SI_MWS_Silicon}. These measurements are consistent with the minority carrier lifetime in silicon. [TODO]
...
\begin{enumerate}
    \item A profile of the potassium atom density from the CFD data along the beam path is obtained as shown in Figures \ref{fig:SI_cfd_beam_goldi_pos_kwt} and \ref{fig:SI_cfd_beam_pos_dep}. This profile is normalized to the climum value to give $f_{K, CFD} (s)$. s represents the distance along the beam path.
