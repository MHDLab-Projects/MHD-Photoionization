...
\label{fig:SI_5x6_pos_mwt_AStime_phi}%DN: Are units of stage position here mm?
\end{figure}
...
%DN: I know you are changing up the figures in the main text, so double check that this figure number is correct
...
%DN: I can't decode what the above sentence means.
...
%DN: Throughout this section, you refer to $\Delta AS$ as MWT AS. Are they the same thing?
...
%TODO: Check that all references to a figure have "fig" or "figure" before them.
...
\begin{figure}
    \centering
    \includegraphics[]{\repodir/final/figure_panels/output/figures/536_iccd_laserspot_zoom.png} 
...
\label{fig:SI_iccd_cfd_comparison}%DN: Double-check that reference to main text figure has the correct number
\end{figure}
...
\caption{Effective recombination coefficient for seed ramp at the nominal position of 180 mm downstream of the barrel exit. a) The exponential time constant measured from the MWT mono-molecular regime, averaged over runs. B) The CFD-predicted total and oxygen number density. C) The effective recombination coefficient calculated from Eq.\ \ref{eq:krbeff}}%DN: Averaged over runs. Which runs?
...
\label{fig:SI_mwt_536_pos_mnum_sample}%DN: First time you mention mnum. What does it mean?
\end{figure}
...
We explored an alternative fitting method for the $\Delta AS$ time profiles by directly solving Eq.\ \ref{eq:full_deqn_phi} numerically using $G_{NE} = 0$. This method should account for the bimolecular recombination term in Eq.\ \ref{eq:full_deqn_phi}. For simplicity and robust results, we limit ourselves to exponential fitting in the main text, but this numerical fitting procedure should be explored further. %DN: This references an equation that comes way later. Should this section be moved after the referenced equation?
...
\begin{figure}
\centering
\includegraphics[]{\repodir/experiment/analysis/mwt/output/figures/mwt_fitting_compare.png}
...
\begin{figure}[]
\centering
\includegraphics[width=0.7\textwidth]{\repodir/final/dataset/output/figures/53x_mwt_fit_dnedtv2.png}
\caption{MWT measurements and fits for 53x for the differential equation solving method. }
\label{fig:SI_53x_mwt_fit_dnedtv2}%DN: What is the y-axis here?
\end{figure}
...
\end{figure}%DN: What is the y-axis here?
