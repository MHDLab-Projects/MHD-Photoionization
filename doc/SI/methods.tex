...
Even with interpolation, slight differences between the calibration spectrum and off-peak free-jet $\alpha(\lambda)$ measurements are observed. For some measurements, this offset is different on different sides of the peak in $\alpha(\lambda)$ (see Fig. \ref{fig:SI_5x3_pos_alpha}). The cause of this asymmetric offsets is unknown and we have not developed a method to correct for asymmetric offsets. For constant offsets, we we perform an offset procedure that maintains the overall shape of $\alpha(\lambda)$ near the high limit of the absorption data (i.e. the offset removal does not affect absorption values near 1). The offset procedure is as follows:

\begin{enumerate}
    \item Calculate $\beta = -\ln(1 - \alpha)$, which represents the exponential component of the Beer-Lambert law. 
    \item Subtract $\beta$ in a in a region of 750 to 755 nm where the absorption is expected to be zero. %DN: Subtract beta "from $\alpha$"?
...
\begin{figure}[]
    \centering
    \includegraphics[width=0.8\textwidth]{\repodir/final/figures/SI/output/AES_barrel_cases.png}
