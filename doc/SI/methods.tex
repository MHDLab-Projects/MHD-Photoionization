...
Throughout the experimental campaign, we performed numerous test case sequences. Test case sequences which are relevant to this work are presented below, identified with our internal naming scheme. Our naming scheme begins with a three-digit number that represents the nominal values for the HVOF process inputs, as follows: The first digit represents the total mass flow (5 = \SI{13.0}{\gram\per\second}), the second digit represents the equivalence ratio (1 = 0.6, 3 = 0.8), and the third digit represents the potassium mass fraction (3 = 0.1\% and 6 = 1.0\%). An "x" indicates that the nominal value was varied during that sequence. Then, the sequence is appended with "pos" or "pow" to indicate a position or power sweep, respectively. The absence of "pos" or "pow" indicates that the SFR-maximized position of \SI{180}{\milli\meter} downstream of the barrel exit and full laser power (\SI{7}{\milli\joule}) were used. Finally, a given test case sequence is appended with a "run number" to indicate repeats of the sequence performed on the same day. 
...
% What does "diagnostic" mean here? Is it equivalent to "measurement"? Or is it 
...
%Check that my added detail about what "both locations" are is correct.
%"Therefore, for a given test case sequence, the MWT diagnostic acquires more data than the AES diagnostic." 
The volume of the syringe pump limits the duration of a given test case sequence, as a higher potassium mass fraction necessitates a higher volume flow of liquid seed. Furthermore, it was not possible to maintain consistent durations for identical test cases performed in different sequences. %Why? It'd be a pain in the ass but couldn't it have been done? Though, I don't see how it makes a difference, which we should probably mention here.
The time limitation on seeding duration limits the AES diagnostic more than the MWT diagnostic. The AES diagnostic must wait \SI{1}{\second} between LED switching events to allow for the LED to stabilize. Additionally, the multiplexer switches every \SI{10}{\second}. Therefore, a minimum of at least \SI{20}{\second} is needed for an $n_K$ measurement at both the exit of the barrel and the center of the microwave horns. Ideally, multiple measurements are taken at each location to ensure reproducibility. %Were you able to take multiple AES measurements at each location for all test cases discussed in the main text?
Conversely to the AES diagnostic, the MWT diagnostic is continuously acquiring data at the repetition rate of the laser 5-(\SI{12}{\hertz}). Therefore, for a given test case sequence, the MWT diagnostic acquires more data than the AES diagnostic. %Why is this important to mention?
For the standard test case sequences outlined above, approximately \SI{1}{\minute} was used for each test case and data is presented for both diagnostics. %Why 1 minute? because there's enough seed to do 1 minute at all higher K mass fraction?
...
Below are overviews of the experiments performed. %Sequences? Also, not just the ones discussed in the main text, so is this every campaign you did?
The independent variables that were varied throughout the experiments are shown, and the colored regions are test case sequences. Times are in PST. Note that not all test case sequences that were performed were analyzed in the main text. 

%See rewrite below
Figures \ref{fig:SI_expt_overview_2023-04-07}-\ref{fig:SI_expt_overview_2023-05-24} show all test case sequences performed for this work. Each test case sequence is color-coded by the independent variables that were varied throughout that sequence (see each figure's legend). The independent variables are potassium mass fraction, equivalence ratio, stage position, laser power, experiment date, run number, and acquisition number. To illustrate which variable is being varied during each test case sequence, each subplot corresponds to one of these independent variables listed above. The test case sequences are shown in the context of the entire experiment.


%Figure S2 is missing a legend.
...
\begin{enumerate}
    ...
    \item The data is resampled, averaging groups of 10 data points to a grid with 0.1 $\mu s$ spacing. %Consider using the siunitx package for units. I can convert what's here to the correct syntax if you want to go that route.
...
For the exponential fit the data is down selected to a window of \SI{5}{\micro\second} to \SI{15}{\micro\second} after the laser pulse and fit with an exponential fit model $y = A \exp(-t/\tau) $.
...
On each date, the microwave transmission magnitude with nothing in between the horns, $U_{nothing}$, was measured. During these measurements, all other settings are the same (excimer laser, oscilloscope, etc.) compared to the main experimental measurements, except that the oscilloscope time window may be 250 $\mu s$ used for the Silicon measurements may have been used. %The oscilloscope time window may be 250 us for the Silicon measurements? Not sure what this means exactly
...
The  measured at the nominal position (\SI{180}{\milli\meter}) for each of the dates is presented in \ref{fig:SI_MWT_nothing_T0}A. %The what measured at the nominal position?
...
Figure \ref{fig:SI_MWT_nothing_T0}B shows the magnitude of absolute transmission before the laser pulse for the case 536\_pos, taken on multiple dates. The transmission is measured with the sample in place. %I thought this measurement was with nothing between the horns?
...
The laser profile was measured by terminating the laser beam on a white beam block and imaging with a Thorlabs DCC1240C-HQ camera. %beam block = Just a white piece of paper or something specific from ThorLabs?
...
%Reference equation in the main paper?
%This sentence is awkward but I'm not sure exactly how to clarify what's being said
%OK to include package csquotes to fix backwards ’ ’ quotes?
In Figure \ref{fig:SI_MWT_lin_square_compare}, we compare the transmission calculated from the microwave signal magnitude ($U$) squared, as done in the paper with the same expression but measuring the "magnitude transmission" $T  = \frac{U - U_{block}}{U_0 - U_{block}}$, where $U_0$ and $U_{block}$ are the magnitude with no object and with blocked receiver, as discussed in the main text. The exponential decay time of $\Delta AS$ can be seen to be invariant to whether power or magnitude transmission is used for calculation. 
...
For the barrel exit measurement, the light from the LED is collimated with a RC02SMA-P01 reflective parabolic mirror and directed at a \SI{90}{\degree} angle with a MRA05-E3 dielectric mirror. After passing through the torch, the light is then de-collimated with a second RC02SMA-P01 reflective parabolic mirror. The light is sent from the multiplexer to the collimating lens with a Thorlabs M38L02 \SI{200}{\micro\meter} 0.39 NA fiber, and sent from the collection lens to the multiplexer with a Thorlabs M53L02 \SI{600}{\micro\meter} 0.5NA fiber. On both sides of the torch, light passes through an iris of approximately \SI{1}{\milli\meter} and a glass slide which is used to protect the optics from the torch. The beam is positioned to go through the center of the torch at approximately 1/8 inch \pm 1/16 inch downstream of the barrel exit. The beam diameter is approximately 3/32 inch to 4/32 inch. %DN: What is the part called that terminates the fiber before it passes through the collimating mirror
...
The LED is continuously turned on and off throughout the experiment to obtain "LED on" and "LED off" measurements. The LED is triggered by the spectrometer. When switching the LED, there is a \SI{1}{\second} delay before the spectrometer is triggered to allow the LED state to stabilize. This LED switching time limits the temporal resolution of the AES measurements. Furthermore, fluctuations in the free jet seed level between the LED on and off measurements can cause nonphysical results, such as negative absorption. In the future, we plan to modify the liquid seeding system to improve the consistency of liquid seeding over time. %DN: Should we point to fig. S20 as an example of negative absorption at some wavelengths? Does this not matter because it is far outside the wavelengths of interest?
...
Before and after each experiment, calibration spectra are taken with no free jet in place. The calibration is required to determine the absorption $\alpha(\lambda)$, as described in the main text. Unfortunately, the calibration spectrum cannot be obtained throughout the experiment as turning off and restarting the HVOF torch is unreliable, so we elect to run the torch continuously through each experiment.

In this work, we develop a method to correct the calibration spectra using the "LED on" and "LED off" measurements. Figure \ref{fig:SI_absem_calib_dates} shows the calibration spectrum for each date. The calibration spectra are derived from the difference between the "LED on" and "LED off" measurements (similarly to the absorption data). For the position dependent measurements, the calibration spectra are taken at the nominal position. %DN: Is the nominal position always 180 mm, as mentioned previously? If so, for clarity we should say "nominal position (180 mm from the barrel exit)" every time "nominal position" is used

We observe changes in the calibration spectra throughout the experiment which cause offsets in the absorption spectra $\alpha(\lambda)$. For the current work, we interpolate the calibration spectra shown in figure \ref{fig:SI_absem_calib_dates} to the time of any given measurement to determine the calibration spectrum for that measurement. In future work, we plan to measure calibration during an experiment by vertically translating the AES diagnostic location.
...
Even with interpolation, slight differences between the calibration spectrum and off-peak free-jet $\alpha(\lambda)$ measurements are observed. For some measurements, this offset is different on either side of the peak in $\alpha(\lambda)$ (see Fig. \ref{fig:SI_5x3_pos_alpha}). The cause of this asymmetric offset is unknown, and we have not developed a method to correct for asymmetric offsets. For constant offsets, we perform an offset procedure that maintains the overall shape of $\alpha(\lambda)$ near the high limit of the absorption data (i.e. the offset removal does not affect absorption values near 1). The offset procedure is as follows:

\begin{enumerate}
    \item Calculate $\beta = -\ln(1 - \alpha)$, which represents the exponential component of the Beer-Lambert law. 
    % \item Subtract $\beta$ in a region of 750 to 755 nm where the absorption is expected to be zero. %DN: Subtract beta "from $\alpha$"?
    \item Recalculate $\alpha = 1 - e^{- (\beta - \beta_{off})}$ 
\end{enumerate}

The effect of the offset procedure is shown in Fig.\ \ref{fig:SI_AES_proc_overview}C. Additional processing steps are as follows:
...
We model $\alpha(\lambda)$ to determine the centerline potassium density, $n_{K, cl}$, by numerically integrating the Beer-Lambert law along the path $s$ of the beam for the wavelength-dependent intensity, $I(\lambda)$
...
\begin{enumerate}
    ...
    \item The potassium atom density profile is converted to $\mu_{atten} (s, \lambda) = n_{K, cl}* f_{K, CFD} (s) \kappa(\lambda)$. $n_{K, cl}$ is the fit parameter corresponding to the centerline (maximum) potassium atom density in the profile.
    \item For each $\lambda$, the intensity profile $I(s)$ is calculated along the beam path with the Euler method: $I(s + \Delta s) = I(s) + \mu_{atten}(s, \lambda) I(s) \Delta s$. The initial condition is $I(0) = 1$. If the intensity becomes negative, the calculation is stopped and the absorption is set to 1. An example of the Euler method is shown in Fig.\ \ref{fig:SI_euler_method_tophat_demo}.
...
\begin{figure}[]
    ...
    \caption{CFD beam profiles used for numerical fitting for the 53x test case sequence.  } %DN: mp = "motor position"?
...
There is a discrepancy between the barrel AES $nK_{max}$ measurements and the CFD centerline $nK_{m3}$ predictions. We investigated the possibility that this discrepancy is due to an incorrect spatial prediction of CFD pressure or $nK$ profile along the beam path in Figure \ref{fig:SI_AES_barrel_cases}.  
...
Figure \ref{fig:SI_PD_setup_schematic} shows a schematic of the photodiode (PD) setup. Fig.\ \ref{fig:SI_PD_setup_image} shows an image taken of the setup. All parts were ordered from Thorlabs. The light is collected above the torch with a F240SMA-B fiber coupled lens (FCL) into an M29 L02 \SI{600}{\micro\meter} 0.39 NA fiber. The collection lens is located (with respect to the measurement position) approximately 6.5 inch above the torch, 1 inch to the left (along the flow axis), and 7/8 inch upstream along the torch axis. The light from this fiber is then collimated into the beam splitter with F110SMA-780 fiber coupled lens with \SI{780}{\nano\meter} f = \SI{6.24}{\milli\meter} and NA = 0.37 SMA. The fiber coupled lenses are connected to a Thorlabs M29L02 \SI{600}{\micro\meter} 0.39 NA optical fiber. The light is split with a CCM1-BS014 50-50 beam splitter (BS) into two separate photodiode pathways that are intended to collect the potassium emission peak and the emission off-peak for comparison. Each pathway consists of a bandpass filter and placed before a SM05PD1A cathode grounded silicon photodiode. There is a P2000HK 2 mm fixed size iris before the photodiode. The photodiode is unbiased. The photodiode signals are amplified with AMP101 transimpedance amplifiers set to an amplification of \SI{100}{\kilo\volt/\ampere}. This signal is then sent to channels 3 and 4 of the oscilloscope, where channels 1 and 2 are used for the MWT I and Q measurements.  FB770-10 (\SI{770(10)}{\nano\meter}) and (FB840-10 \SI{840(10)}{\nano\meter}) bandpass filters are placed before the on-peak and off-peak measurements, respectively. 
