...
def struct_mesh(x):
    ...
    """
    return pv.StructuredGrid(*split_dims(x))
...
def flat_transform(x0, shat, x_in, method="matmul", verbose=False):
    ...
    """
    if (verbose):
        print(x0.shape, shat.shape, x_in.shape)
    x_out = np.matmul(x_in[:,:],shat[:,:])
...
def test():
    ...
    #
    # beam perturbation to get  the local jet width and spreading rate
    #
    sub_dy = 3.0e-3
    sub_dx = 5.0e-3
