...
ds_notorch['dAS_abs'].mwt._pulse_max().mean('run').plot(ax=axes[0])
axes[0].set_title('No Torch')

ds_5x0['dAS_abs'].mwt._pulse_max().mean('run').plot(hue='phi', ax=axes[1])
axes[1].set_title('5x0')
ds_5x3['dAS_abs'].mwt._pulse_max().mean('run').plot(hue='phi', ax=axes[2])

axes[2].set_title('5x3')

ds_5x6['dAS_abs'].mwt._pulse_max().mean('run').plot(hue='phi', ax=axes[3])

axes[3].set_title('5x6')


ds_536['dAS_abs'].mwt._pulse_max().mean('run').plot(hue='phi', ax=axes[4])
...
ds_notorch['dAS_abs'].mwt._pulse_max().mean('run').plot(label='No Torch')
ds_5x0['dAS_abs'].mwt._pulse_max().mean('run').sel(phi=0.8, method='nearest').plot(label='5x0')
ds_5x3['dAS_abs'].mwt._pulse_max().mean('run').sel(phi=0.8, method='nearest').plot(label='5x3')

ds_536['dAS_abs'].mwt._pulse_max().mean('run').plot(label='536')
...
# da_mean = ds_notorch['dAS_abs'].mwt._pulse_max().isel(run=0).mean('mnum')
# da_std = ds_notorch['dAS_abs'].mwt._pulse_max().isel(run=0).std('mnum')

# xr_errorbar_axes(da_mean, da_std, label='No Torch', axes=axes)

# da_mean = ds_5x0['dAS_abs'].mwt._pulse_max().isel(run=0).sel(phi=0.8,method='nearest').mean('mnum')
# da_std = ds_5x0['dAS_abs'].mwt._pulse_max().isel(run=0).sel(phi=0.8,method='nearest').std('mnum')

# xr_errorbar_axes(da_mean, da_std, label='5x0', axes=axes)

# da_mean = ds_5x3['dAS_abs'].mwt._pulse_max().isel(run=0).sel(phi=0.8,method='nearest').mean('mnum')
# da_std = ds_5x3['dAS_abs'].mwt._pulse_max().isel(run=0).sel(phi=0.8,method='nearest').std('mnum')

# xr_errorbar_axes(da_mean, da_std, label='5x3', axes=axes)

# da_mean = ds_536['dAS_abs'].mwt._pulse_max().mean('mnum').mean('run')
# da_std = ds_536['dAS_abs'].mwt._pulse_max().std('mnum').mean('run')
...
ds_536 = ds_536.mwt.calc_time_stats()
...
dAS_max = ds_536['dAS_abs'].mwt._pulse_max().sel(run=[('2023-05-18', 1), ('2023-05-18', 2)]).mean('mnum')
