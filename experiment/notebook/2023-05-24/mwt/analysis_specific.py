...
ds_mwt_fit, ds_p, ds_p_stderr = pipe_fit_mwt_1(da_fit, pre_norm_cutoff=None)
...
tc_vals = ds_mwt_fit.coords[tc_dim]
...
for i, tc_val in enumerate(tc_vals):
    ...
    ds_mwt_fit['AS'].sel({tc_dim: tc_val}).plot(marker='o', label='Data', ax=ax)
    ds_mwt_fit['AS_fit'].sel({tc_dim: tc_val}).plot(linestyle='--', label='Fit', color='red', ax=ax)
