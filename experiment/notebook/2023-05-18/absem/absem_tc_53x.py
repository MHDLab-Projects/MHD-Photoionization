...
data_folder = mhdpy.io.gen_path('sharepoint', 'Data Share', 'MHD Lab', 'HVOF Booth', '2023-05-18')

dsst = mhdpy.io.TFxr(pjoin(data_folder, 'Processed_Data.tdms')).as_dsst()

ds = xr.load_dataset(pjoin(DIR_PROC_DATA, 'ds_alpha.cdf'))


ds_mp = ds

tw = slice(Timestamp('2023-05-18 21:35:36.851033344'), Timestamp('2023-05-18 22:11:31.644259072'), None)
...
ds_mp['led_off'].mean('mnum').plot(hue='kwt', col='mp')


    # ds_sel.coords['motor'].attrs = dict(long_name='kwtition', units = 'mm')

# %%

g = ds_mp.to_array('var').sel(wavelength=slice(700,785)).mean('mnum').plot(hue='kwt', col='mp', row='var', sharey=False)
...
# %%
g = ds_alpha[['alpha','alpha_fit']].sel(mp='mw_horns').to_array('var').plot(col='kwt',hue='var', ylim = (1e-3,2), yscale='log', figsize=(10,6), col_wrap=3)
