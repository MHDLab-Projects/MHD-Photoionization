...
timeoffset_1 = setupfile_timeoffset(pjoin(lecroy_setup_folder, r'Setup_2023_05_12_13_22_06_740182--00000.lss'))
...
for dirpath, dirnames, filenames in os.walk(lecroy_raw_folder):
    if len(filenames) > 3:
        ...
        try:
            mnums = get_matching_filepath_mnums(filenames, channels=['1','2'])
        ...
        for mnum_list in chunks(mnums, chunk_size):
            ...
            try:
                ds = load_trc_mnum_simple(dirpath, mnum_list, channels=['1', '2'], time_offset=time_offset)
            except NoMatchingFilesError as e:
                print("No Matching files in batch, skipping batch") 
