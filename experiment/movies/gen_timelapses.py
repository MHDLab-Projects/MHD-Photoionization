...
for date in dates:
    ...
    ts_start = Timestamp(df_ct.loc[date]['Start Time'], tz='UTC').tz_convert('US/Pacific').tz_localize(None)
    ts_stop = Timestamp(df_ct.loc[date]['Stop Time'], tz='UTC').tz_convert('US/Pacific').tz_localize(None)
    ...
    folder_out = os.path.join('output', 'flir', date.strftime("%Y-%m-%d"))
    ...
    for channel in channels:
        ...
        fn_out = 'Timelapse_{}_ch{}.mp4'.format(date.strftime("%Y-%m-%d"), channel)
