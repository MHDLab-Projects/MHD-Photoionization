...
ds_lecroy_536 = ppu.fileio.load_lecroy(tc, norm_mag=True).expand_dims('phi')

tc = '516_pos'

ds_absem_536 = ppu.fileio.load_absem(tc).expand_dims('phi')
ds_lecroy_516 = ppu.fileio.load_lecroy(tc, norm_mag=True).expand_dims('phi')
...
da_fit = ds_lecroy['AS_abs'].mean('mnum')
