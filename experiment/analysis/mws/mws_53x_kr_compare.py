...
rxn_rates = {
    ...
    'O2': Quantity(1e-30, 'cm^6/s')*ds_cfd['rho'],
    # 'O2': Quantity(6e-34, 'cm^6/s')*ds_cfd['rho'],
    'H2O': Quantity(1e-8, 'cm^3/s')*np.exp(-(Quantity(35000, 'K')/ds_cfd['T'])),
}
...
ds_species_decay = 1/(2*ds_p['decay'].pint.quantify('us')*ds_kr)
...
ds_tau = 1/(2*ds_species_cfd*ds_kr)
...
ds_tau.to_array('species').plot(hue='species', marker='o')
